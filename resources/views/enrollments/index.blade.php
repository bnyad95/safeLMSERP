<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ __('Module Offerings') }}</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Open catalog courses for an academic year, stage, semester, group, and teacher.') }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('course-records.index') }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">{{ __('Course Catalog') }}</a>
                @if($abilities['manage'])
                    <a href="{{ route('course-sections.archived') }}" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">{{ __('Archived') }} {{ $archivedCount ? '('.$archivedCount.')' : '' }}</a>
                    <a href="{{ route('course-sections.create') }}" class="inline-flex items-center rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 dark:bg-blue-600 dark:hover:bg-blue-500">{{ __('Add Module Offering') }}</a>
                @endif
            </div>
        </div>
    </x-slot>
            ...
            <div class="grid gap-4 md:grid-cols-4">
                @foreach([
                    ...
                    <div class="{{ $stat['tone'] }} rounded-lg border border-gray-200 p-5 shadow-sm dark:border-gray-700">
                        <div class="text-sm font-medium text-gray-600 dark:text-gray-300">{{ __($stat['label']) }}</div>
            ...
            <form method="GET" action="{{ route('module-offerings.index') }}" class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900" x-data="{ showMore: {{ $hasAdvancedOfferingFilters ? 'true' : 'false' }} }">
                <div class="grid gap-4 md:grid-cols-4">
                    <div class="md:col-span-2">
                        ...
                        <input name="q" value="{{ $filters['q'] }}" class="mt-1 block w-full rounded-md border-gray-300 bg-white text-sm text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" placeholder="{{ __('Course code, name, or group') }}">
                    </div>
                    <div>
                ...
                <div class="mt-4 flex justify-end gap-2">
                    <a href="{{ route('module-offerings.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Reset') }}</a>
                    <button class="rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 dark:bg-blue-600 dark:hover:bg-blue-500">{{ __('Apply') }}</button>
                </div>
            </form>
            ...
            @if($classificationGroups->isNotEmpty())
                <div class="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
                    <div class="border-b border-gray-200 px-5 py-4 dark:border-gray-700">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Classification') }}</h3>
                    </div>
                            ...
                            <div class="p-5">
                                <div class="flex flex-wrap items-center justify-between gap-3">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ __(':count modules, :students enrolled', ['count' => $college['count'], 'students' => $college['students']]) }}</div>
                                    </div>
                                </div>
                                <div class="mt-4 grid gap-3 md:grid-cols-3">
                                    @foreach($college['departments'] as $department)
                                        <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                                            ...
                                            <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __(':count modules, :students students', ['count' => $department['count'], 'students' => $department['students']]) }}</div>
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                @forelse($sections as $section)
                    ...
                    <a href="{{ route('course-sections.show', $section) }}" class="block rounded-lg border border-gray-200 bg-white p-5 shadow-sm transition hover:border-gray-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-900 dark:hover:border-gray-600">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <div class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $section->semester->name }} {{ $section->semester->academic_year }} / {{ $section->programSemesterLabel() }} / {{ __('Group :code', ['code' => $section->section_code]) }}</div>
                                <div class="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $section->course->code }} - {{ $section->course->name }}</div>
                            </div>
                            <span class="rounded-md px-2 py-1 text-xs font-semibold {{ $section->status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-200' : ($section->status === 'closed' ? 'bg-gray-200 text-gray-700' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-200') }}">{{ __(ucfirst($section->status)) }}</span>
                        </div>
                        <dl class="mt-5 grid grid-cols-2 gap-3 text-sm">
                            <div>
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('College') }}</dt>
                                <dd class="font-medium text-gray-900 dark:text-gray-100">{{ $section->course->department->college->name ?? __('No college') }}</dd>
                            </div>
                            <div>
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('Department') }}</dt>
                                <dd class="font-medium text-gray-900 dark:text-gray-100">{{ $section->course->department->name ?? __('No department') }}</dd>
                            </div>
                            <div>
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('Stage') }}</dt>
                                <dd class="font-medium text-gray-900 dark:text-gray-100">{{ $section->grade_level ?: __('No stage') }}</dd>
                            </div>
                            <div>
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('Program Semester') }}</dt>
                                <dd class="font-medium text-gray-900 dark:text-gray-100">{{ $section->programSemesterLabel() }}</dd>
                            </div>
                            <div>
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('Teacher') }}</dt>
                                <dd class="font-medium text-gray-900 dark:text-gray-100">{{ $section->teacher->full_name ?? __('Unassigned') }}</dd>
                            </div>
                        </dl>
                        <div class="mt-5">
                            <div class="flex justify-between text-xs font-medium text-gray-600 dark:text-gray-300">
                                <span>{{ __(':enrolled / :capacity enrolled', ['enrolled' => $section->enrolled_count, 'capacity' => $section->capacity]) }}</span>
                                <span>{{ __(':count waitlisted', ['count' => $section->waitlisted_count]) }}</span>
                            </div>
                ...
                @empty
                    <div class="rounded-lg border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 lg:col-span-3">
                        {{ __('No modules match the current filters.') }}
                    </div>
                @endforelse
            </div>
