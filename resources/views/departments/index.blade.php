<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Departments</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400">Define and manage departments mapped to universities and colleges.</p>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        @forelse($departments as $department)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $department->code ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $department->university->name ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $department->college->name ?? 'N/A' }}</td>
                                @if($canManageDepartments)
