...
<div class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
    <section>
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Teacher Information') }}</h3>
        <div class="mt-4 grid gap-5 md:grid-cols-2">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Teacher ID') }}</label>
                @if($isEdit)
                ...
                @else
                    <p class="mt-1 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">{{ $suggestedStaffId ?? __('Will be generated on save') }}</p>
                @endif
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('This ID is generated by the system.') }}</p>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $teacher->title ?? '') }}" placeholder="{{ __('Lecturer, Assistant Professor, Professor') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:placeholder-gray-500">
    ...
    <section>
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Organization Placement') }}</h3>
        <div class="mt-4 grid gap-5 md:grid-cols-2">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('University') }} <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('College') }} <span class="text-red-500">*</span></label>
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Department') }} <span class="text-red-500">*</span></label>
    ...
    <section>
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Employment and Access') }}</h3>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Inactive and retired teachers cannot use teacher login access.') }}</p>
        <div class="mt-4 max-w-xs">
            ...
            <select name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                @foreach(['Active','Inactive','Retired'] as $statusOption)
                    <option value="{{ $statusOption }}" @selected(old('status', $teacher->status ?? 'Active') === $statusOption)>{{ __($statusOption) }}</option>
                @endforeach
            </select>
    ...
    @unless($isEdit)
        <section>
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Teacher Login') }}</h3>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Set a temporary password. The teacher must change it after first login.') }}</p>
            <div class="mt-4 grid gap-5 md:grid-cols-2">
                <div>
    ...
    <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-5 dark:border-gray-800">
        <a href="{{ isset($teacher) ? route('teachers.show', $teacher) : route('teachers.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Cancel') }}</a>
        <button type="submit" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 dark:bg-indigo-600 dark:hover:bg-indigo-500">{{ $isEdit ? __('Update Teacher') : __('Create Teacher') }}</button>
    </div>
</div>
