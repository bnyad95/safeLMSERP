@forelse($participants as $participant)
    <a href="{{ route('class-messages.index', ['courseSection' => $courseSection, 'recipient_id' => $participant->id]) }}" class="flex items-center gap-3 px-4 py-4 {{ $selectedUser?->id === $participant->id ? 'bg-blue-50' : 'hover:bg-gray-50' }}">
        ...
        <div class="min-w-0 flex-1">
            ...
            <p class="mt-0.5 text-xs text-gray-500">{{ auth()->user()->hasRole('student') ? 'Teacher' : ($participant->academic_id ?: 'Student') }}</p>
        </div>
...
@empty
    <p class="px-4 py-8 text-sm text-gray-500">No messaging contacts are available for this class.</p>
@endforelse
