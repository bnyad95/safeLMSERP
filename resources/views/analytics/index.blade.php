<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-gray-900">{{ __('Institution Analytics') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ $canViewFinanceAnalytics ? __('Academic, attendance, finance, and course performance signals.') : __('Academic, attendance, and course performance signals.') }}</p>
            </div>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('analytics.index') }}" class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
                    <div class="min-w-0">
                        <label class="block text-sm font-medium text-gray-700">{{ __('College') }}</label>
                    ...
                    <div class="min-w-0">
                        <label class="block text-sm font-medium text-gray-700">{{ __('Semester') }}</label>
                ...
                <div class="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
                    <a href="{{ route('analytics.index', ['tab' => $activeTab]) }}" class="inline-flex justify-center rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">{{ __('Reset') }}</a>
                    <button type="submit" class="rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">{{ __('Apply Filters') }}</button>
                </div>
            </form>
            ...
            @if($activeTab === 'academic')
                <section class="rounded-lg border border-gray-200 bg-white shadow-sm">
                    <div class="border-b border-gray-200 px-5 py-4">
                        <h3 class="text-base font-semibold text-gray-900">{{ __('GPA Trend') }}</h3>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('GPA') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Average Mark') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Published Marks') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-5 py-8 text-center text-sm text-gray-500">{{ __('No published marks available for GPA trend.') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            @if($activeTab === 'attendance')
                <section class="rounded-lg border border-gray-200 bg-white shadow-sm">
                    <div class="border-b border-gray-200 px-5 py-4">
                        <h3 class="text-base font-semibold text-gray-900">{{ __('Attendance Risk') }}</h3>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Rate') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Absences') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Risk') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                @forelse($attendanceRisk as $row)
                                    <tr>
                                        ...
                                        <td class="px-5 py-3">
                                            <span class="rounded-md {{ $row['risk'] === 'High' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700' }} px-2 py-1 text-xs font-semibold">{{ __($row['risk']) }}</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-5 py-8 text-center text-sm text-gray-500">{{ __('No attendance risk detected.') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            @if($activeTab === 'finance')
                <section class="rounded-lg border border-gray-200 bg-white shadow-sm">
                    <div class="border-b border-gray-200 px-5 py-4">
                        <h3 class="text-base font-semibold text-gray-900">{{ __('Unpaid Balances') }}</h3>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Student') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Balance') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Overdue') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Last Activity') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-5 py-8 text-center text-sm text-gray-500">{{ __('No unpaid balances found.') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Average') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Pass Rate') }}</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Attendance') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-5 py-8 text-center text-sm text-gray-500">{{ __('No course performance data yet.') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
