...
<div class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
    ...
    <div class="grid gap-6 md:grid-cols-2">
        <div>
            <label for="semester-university-id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">University</label>
        ...
        <div>
            <label for="semester-academic-year" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Academic year</label>
            <select id="semester-academic-year" name="academic_year_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100" required @disabled($academicYears->isEmpty())>
                ...
                @foreach($academicYears as $academicYear)
                    <option value="{{ $academicYear->id }}" data-university-id="{{ $academicYear->university_id }}" data-status="{{ $academicYear->status }}" @selected($selectedAcademicYear == $academicYear->id)>{{ $academicYear->name }} / {{ ucfirst($academicYear->status) }}</option>
                @endforeach
            </select>
            @if($academicYears->isEmpty())
                <p class="mt-2 text-sm text-amber-700 dark:text-amber-300">Create an academic year first. <a href="{{ route('academic-years.create') }}" class="font-semibold underline">Add Academic Year</a></p>
            @endif
        </div>
        ...
        <div>
            ...
            <select id="semester-term-type" name="term_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100" required>
                <option value="regular" @selected($selectedType === 'regular')>Regular semester</option>
                <option value="summer" @selected($selectedType === 'summer')>Summer semester</option>
            </select>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Sequence</label>
    ...
    <div class="flex items-center justify-end gap-3">
        <a href="{{ $selectedAcademicYear ? route('academic-years.show', $selectedAcademicYear) : route('semesters.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">Cancel</a>
        <button type="submit" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700" @disabled(! $isEdit && $academicYears->isEmpty())>{{ $isEdit ? 'Update Semester' : 'Create Semester' }}</button>
    </div>
</div>
