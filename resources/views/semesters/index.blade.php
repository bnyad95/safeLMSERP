<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800">Semesters</h2>
                <p class="text-sm text-gray-600">Define semester terms and academic periods.</p>
            </div>
            <div class="flex gap-2">
                ...
                @if($canManageSemesters)
                    ...
                    <a href="{{ route('semesters.create') }}" class="inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Add Extra Semester</a>
                @endif
            </div>
        </div>
    </x-slot>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($semesters as $semester)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $semester->academic_year }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $semester->university->name ?? 'N/A' }}</td>
