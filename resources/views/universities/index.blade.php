<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800">Universities</h2>
                <p class="text-sm text-gray-600">Define and manage university master records.</p>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($universities as $university)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $university->email ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $university->phone ?? 'N/A' }}</td>
                                @if($canManageUniversities)
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $canManageUniversities ? 5 : 4 }}" class="px-6 py-8 text-center text-sm text-gray-500">No universities defined yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
