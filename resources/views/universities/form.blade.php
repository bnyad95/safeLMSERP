...
<div class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
    <div class="grid gap-6 md:grid-cols-2">
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Organization name') }}</label>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Institution type') }}</label>
            <select id="institution-type" name="institution_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100" required>
                <option value="university" @selected($selectedType === 'university')>{{ __('University') }}</option>
                <option value="institute" @selected($selectedType === 'institute')>{{ __('Institute') }}</option>
            </select>
        </div>

        <div class="md:col-span-2 border-y border-gray-200 py-4 dark:border-gray-800">
            <p class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('Automatic academic structure') }}</p>
            <div class="mt-3 grid gap-3 text-sm sm:grid-cols-3 sm:divide-x sm:divide-gray-200 dark:sm:divide-gray-800">
                <div><span id="structure-stage-count" class="block text-xl font-semibold text-gray-900 dark:text-white">{{ $selectedStructure['expected_stage_count'] }}</span><span class="text-gray-500 dark:text-gray-400">{{ __('Program stages') }}</span></div>
                <div class="sm:pl-4"><span id="structure-year-semesters" class="block text-xl font-semibold text-gray-900 dark:text-white">{{ $selectedStructure['expected_semesters_per_year'] }}</span><span class="text-gray-500 dark:text-gray-400">{{ __('Regular semesters per year') }}</span></div>
                <div class="sm:pl-4"><span id="structure-program-semesters" class="block text-xl font-semibold text-gray-900 dark:text-white">{{ $selectedStructure['expected_stage_count'] * $selectedStructure['expected_semesters_per_year'] }}</span><span class="text-gray-500 dark:text-gray-400">{{ __('Standard program semesters') }}</span></div>
            </div>
            <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">{{ __('The standard structure is fixed by institution type. Students may repeat a stage or use summer and later-year retake periods without changing the program numbering.') }}</p>
        </div>
    ...
    <div class="flex items-center justify-end gap-3">
        <a href="{{ route('universities.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Cancel') }}</a>
        <button type="submit" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">{{ $isEdit ? __('Update Organization') : __('Create Organization') }}</button>
    </div>
</div>
