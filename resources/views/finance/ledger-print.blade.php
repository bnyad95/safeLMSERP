...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ __('Student Ledger') }} - {{ $student->full_name }}</title>
        ...
        <main class="px-4 py-8">
            ...
            <section class="a4-page mx-auto rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                <div class="flex flex-row items-start justify-between gap-5 border-b border-gray-200 pb-6">
                    <div>
                        <p class="text-sm font-semibold uppercase text-gray-500">{{ __('Student Ledger') }}</p>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500">{{ __('Amount') }}</th>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Approval') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">{{ __('Reason') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500">{{ __('No finance records match the current filters.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
