<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="min-w-0">
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Student Finance') }}</p>
                <h2 class="truncate text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ __('Add Finance Record') }}</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    {{ $selectedStudent->full_name }} / {{ $selectedStudent->student_id }}
                </p>
            </div>
            <a href="{{ route('finance.students.show', $selectedStudent) }}" class="inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 sm:w-auto dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:hover:bg-gray-800">
                {{ __('Back to Student Ledger') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="finance-workspace py-5 sm:py-8">
        ...
        >
            <section class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-5 dark:border-gray-800 dark:bg-gray-900">
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div class="min-w-0">
                        <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Student') }}</p>
                    ...
                    <div class="min-w-0">
                        <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Student ID') }}</p>
                        <p class="mt-1 truncate text-gray-800 dark:text-gray-200">{{ $selectedStudent->student_id }}</p>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('College') }}</p>
                        <p class="mt-1 truncate text-gray-800 dark:text-gray-200" title="{{ $selectedStudent->department?->college?->name ?? __('Not assigned') }}">{{ $selectedStudent->department?->college?->name ?? __('Not assigned') }}</p>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Department') }}</p>
                        <p class="mt-1 truncate text-gray-800 dark:text-gray-200" title="{{ $selectedStudent->department?->name ?? __('Not assigned') }}">{{ $selectedStudent->department?->name ?? __('Not assigned') }}</p>
                    </div>
                </div>
            </section>

                <section class="min-w-0 overflow-hidden rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-5 dark:border-gray-800 dark:bg-gray-900">
                    <div class="min-w-0">
                        <h3 class="text-base font-semibold text-gray-900">{{ __('Finance Record Details') }}</h3>
                        <p class="mt-1 text-sm text-gray-500">{{ __('Invoice and receipt numbers are generated automatically unless you enter one.') }}</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('finance.transactions.store') }}" class="mt-5 grid grid-cols-1 gap-4" x-on:submit="stripMoneyCommas($el)">
                        ...
                        <div class="min-w-0">
                            ...
                            @if($autoBilledPlan)
                                <p class="mt-2 text-xs text-blue-700">{{ __("This student is on an automatically billed tuition plan — invoices are generated automatically at the department's rate. Manual invoice entry is disabled here to prevent conflicting charges.") }}</p>
                            @endif
                        </div>
                        ...
                        <div class="min-w-0" x-show="recordType === 'discount'">
                            ...
                            <select name="discount_mode" x-model="discountMode" class="mt-1 block w-full min-w-0 rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="amount" @selected(old('discount_mode', 'amount') === 'amount')>{{ __('Fixed amount') }}</option>
                                <option value="percentage" @selected(old('discount_mode') === 'percentage')>{{ __('Percentage of invoice') }}</option>
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div class="min-w-0" x-show="!(recordType === 'discount' && discountMode === 'percentage')">
                                <label class="block text-sm font-medium text-gray-700">
                                    <span x-show="recordType === 'invoice'">{{ __('Agreed Tuition Charge') }}</span>
                                    <span x-show="recordType !== 'invoice'">{{ __('Amount') }}</span>
                                </label>
                            ...
                            <div class="min-w-0" x-show="recordType === 'discount' && discountMode === 'percentage'">
                                <label class="block text-sm font-medium text-gray-700">{{ __('Discount Percentage') }}</label>
                                <input type="number" min="0.01" max="100" step="0.01" name="discount_percentage" value="{{ old('discount_percentage') }}" x-model="discountPercentage" placeholder="{{ __('e.g. 10') }}" class="mt-1 block w-full min-w-0 rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500" :required="recordType === 'discount' && discountMode === 'percentage'">
                                <p class="mt-1 text-xs text-gray-500" x-show="selectedInvoiceId">&asymp; <span class="font-semibold" x-text="discountPercentageAmount()"></span> <span x-text="currency"></span> {{ __('off the selected invoice.') }}</p>
                            </div>
                        ...
                        @if($canCreateInvoice)
                            <div x-show="recordType === 'invoice'" class="min-w-0 rounded-md border border-blue-100 bg-blue-50 p-4">
                                <label class="block text-sm font-semibold text-gray-900">{{ __('Student tuition agreement') }}</label>
                                <p class="mt-1 text-xs text-blue-900">{{ __('Enter the agreed tuition charge, choose how the student wants to pay it, and the system will create the correct invoice schedule.') }}</p>
                                @if($selectedStudent->preferred_payment_method === 'per_credit')
                                    <p class="mt-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">{{ __('The registrar recorded this student for per-credit billing. Use "Generate Tuition Charge" on the student\'s ledger instead of this form so charges are computed automatically at the department\'s rate.') }}</p>
                                @elseif($selectedStudent->preferred_payment_method)
                                    <p class="mt-2 text-xs text-blue-800">{{ __('Registrar recorded plan:') }} {{ $selectedStudent->preferred_payment_method === 'semester' ? __('divide tuition by semesters') : __('full tuition paid once') }}{{ $selectedStudent->preferred_installment_count ? ' ('.__(':count installments', ['count' => $selectedStudent->preferred_installment_count]).')' : '' }}.</p>
                                @endif
                                <select name="payment_plan" x-model="paymentPlan" class="mt-3 block w-full min-w-0 rounded-md border-blue-200 bg-white text-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="full" @selected($initialPaymentPlan === 'full')>{{ __('Full tuition paid once') }}</option>
                                    <option value="semester" @selected($initialPaymentPlan === 'semester')>{{ __('Divide tuition by semesters') }}</option>
                                </select>

                                <div x-show="paymentPlan === 'full'" class="mt-4 rounded-md border border-blue-200 bg-white px-3 py-2 text-sm text-blue-900">
                                    <p>{{ __('One tuition invoice will be created for') }} <span class="font-semibold" x-text="formattedAmount()"></span>.</p>
                                    ...
                                    <label class="mt-3 flex items-start gap-2">
                                        ...
                                        <span>
                                            <span class="block font-semibold">{{ __('Collect the full payment now') }}</span>
                                            <span class="block text-xs">{{ $canPostImmediately ? __('Posts immediately and generates a receipt.') : __('Records a payment for independent approval before posting.') }}</span>
                                        </span>
                                    </label>
                                    @endif
                                </div>
                                ...
                                <div x-show="paymentPlan === 'semester'" class="mt-4 space-y-3">
                                    <div class="min-w-0">
                                        <label class="block text-sm font-medium text-gray-700">{{ __('Number of installments (full program length)') }}</label>
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">{{ __("Defaults to :count for this student's program. The total tuition is divided across this many semesters, even ones that don't exist yet.", ['count' => $expectedInstallmentCount]) }}</p>
                                    </div>

                                    <div class="rounded-md border border-blue-200 bg-white px-3 py-2 text-sm text-blue-900">
                                        {{ __('Each installment is') }}
                                        <span class="font-semibold" x-text="perSemesterAmount()"></span>.
                                        <span x-show="selectedSemesters.length > 0">
                                            <span class="font-semibold" x-text="selectedSemesters.length"></span> {{ __('invoice(s) will be created now for the semesters checked below;') }}
                                        </span>
                                        {{ __('the remaining installments will be invoiced automatically as future semesters are created.') }}
                                    </div>
                                        ...
                                        @forelse($semesterOptions as $semester)
                                            <label class="flex min-w-0 items-start gap-2 rounded-md border border-blue-100 bg-white p-3 text-sm {{ $semester->end_date ? '' : 'opacity-60' }}">
                                                ...
                                                <span class="min-w-0">
                                                    ...
                                                    <span class="block text-xs text-gray-500">{{ $semester->end_date ? __('Due :date', ['date' => \Illuminate\Support\Carbon::parse($semester->end_date)->format('Y-m-d')]) : __('Missing semester end date') }}</span>
                                                </span>
                                            </label>
                                        @empty
                                            <span class="text-sm text-blue-900">{{ __("No semesters are defined for this student's university yet.") }}</span>
                                        @endforelse
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div class="min-w-0">
                                <label class="block text-sm font-medium text-gray-700">{{ __('Transaction Date') }}</label>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div class="min-w-0">
                                <label class="block text-sm font-medium text-gray-700">{{ __('Reference') }}</label>
                                <input type="text" name="reference" value="{{ old('reference') }}" placeholder="{{ __('Receipt, invoice, voucher...') }}" class="mt-1 block w-full min-w-0 rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                            <div class="min-w-0">
                        ...
                        <div x-show="recordType === 'invoice'" class="min-w-0">
                            ...
                            <input type="text" name="invoice_number" value="{{ old('invoice_number') }}" placeholder="{{ __('Auto for invoices') }}" class="mt-1 block w-full min-w-0 rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div x-show="['payment', 'discount'].includes(recordType)" class="min-w-0">
                            <label class="block text-sm font-medium text-gray-700">{{ __('Apply Credit To Invoice') }}</label>
                            <select name="invoice_transaction_id" x-model="selectedInvoiceId" :required="recordType === 'discount' && discountMode === 'percentage'" class="mt-1 block w-full min-w-0 truncate rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="">{{ __('No invoice allocation') }}</option>
                                @foreach($invoiceOptions as $invoice)
                                    <option value="{{ $invoice->id }}" @selected(old('invoice_transaction_id') == $invoice->id)>
                                        {{ $invoice->documentNumber() }} / {{ money($invoice->remaining_amount ?? $invoice->amount, $invoice->currency) }} {{ $invoice->currency }} {{ __('remaining') }} / {{ ucfirst($invoice->payment_status) }}
                                    </option>
                                @endforeach
                            </select>
                            <p class="mt-1 text-xs text-gray-500" x-show="!(recordType === 'discount' && discountMode === 'percentage')">{{ __('Used for payments and discounts. Currency and student must match the invoice.') }}</p>
                            <p class="mt-1 text-xs text-gray-500" x-show="recordType === 'discount' && discountMode === 'percentage'">{{ __("Required for percentage discounts — the percentage is applied to this invoice's full amount.") }}</p>
                        </div>
