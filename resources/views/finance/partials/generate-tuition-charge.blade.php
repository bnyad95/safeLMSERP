@if($canCreateInvoice)
    <x-modal name="generate-tuition-charge" max-width="lg" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Generate Tuition Charge') }}</h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __("Compute an invoice from this student's active enrollments for a semester (credits × the department's per-credit rate), or enter an amount to charge manually. Already-charged enrollments are skipped automatically.") }}</p>

            @if($tuitionChargeSemesterOptions->isEmpty())
                <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">{{ __("No active or upcoming semester is available for this student's university.") }}</p>
            ...
            @else
                <form method="POST" action="{{ route('finance.students.tuition-charges.store', $selectedStudent) }}" class="mt-4 grid gap-4 sm:grid-cols-2" x-on:submit="stripMoneyCommas($el)">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ __("Defaults to the semester's end date.") }}</p>
                    </div>
                    <div class="min-w-0">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Amount (optional)') }}</label>
                        <input type="text" inputmode="decimal" name="amount" placeholder="{{ __('Auto-calculate') }}" x-on:input="formatMoneyInput($event)" class="money-input mt-1 block w-full min-w-0 rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ __('Leave blank to auto-calculate from credits × rate.') }}</p>
                    </div>
                    ...
                    <div class="flex justify-end gap-2 sm:col-span-2">
                        <button type="button" x-on:click="$dispatch('close')" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Cancel') }}</button>
