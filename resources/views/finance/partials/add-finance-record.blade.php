    ...
    <x-modal name="add-finance-record" max-width="2xl" focusable :show="$reopenAddRecordModal">
        ...
        >
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Add Finance Record') }}</h2>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Invoice and receipt numbers are generated automatically unless you enter one.') }}</p>
            ...
            <form method="POST" action="{{ route('finance.transactions.store') }}" class="mt-5 grid grid-cols-1 gap-4" x-on:submit="stripMoneyCommas($el)">
                ...
                <div class="min-w-0">
                    ...
                    @if($autoBilledPlan)
                        <p class="mt-2 text-xs text-blue-800 dark:text-blue-300">{{ __("This student is on an automatically billed tuition plan — invoices are generated automatically at the department's rate. Manual invoice entry is disabled here to prevent conflicting charges.") }}</p>
                    @endif
                </div>

                <div class="min-w-0" x-show="recordType === 'discount'">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Discount Type') }}</label>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div class="min-w-0" x-show="recordType === 'discount' && discountMode === 'percentage'">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Discount Percentage') }}</label>
                        <input type="number" min="0.01" max="100" step="0.01" name="discount_percentage" value="{{ old('discount_percentage') }}" x-model="discountPercentage" placeholder="{{ __('e.g. 10') }}" class="mt-1 block w-full min-w-0 rounded-md border-gray-300 bg-white text-sm text-gray-900 focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" :required="recordType === 'discount' && discountMode === 'percentage'">
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" x-show="selectedInvoiceId">&asymp; <span class="font-semibold" x-text="discountPercentageAmount()"></span> <span x-text="currency"></span> {{ __('off the selected invoice.') }}</p>
                    </div>
                ...
                @if($canCreateInvoice)
                    <div x-show="recordType === 'invoice'" class="min-w-0 rounded-md border border-blue-100 bg-blue-50 p-4 dark:border-blue-900/60 dark:bg-blue-950/30">
                        <label class="block text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('Student tuition agreement') }}</label>
                        <p class="mt-1 text-xs text-blue-900 dark:text-blue-200">{{ __('Enter the agreed tuition charge, choose how the student wants to pay it, and the system will create the correct invoice schedule.') }}</p>
                        @if($selectedStudent->preferred_payment_method === 'per_credit')
                            <p class="mt-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">{{ __('The registrar recorded this student for per-credit billing. Use "Generate Tuition Charge" instead so charges are computed automatically at the department\'s rate.') }}</p>
                        @elseif($selectedStudent->preferred_payment_method)
                            <p class="mt-2 text-xs text-blue-800 dark:text-blue-300">{{ __('Registrar recorded plan:') }} {{ $selectedStudent->preferred_payment_method === 'semester' ? __('divide tuition by semesters') : __('full tuition paid once') }}{{ $selectedStudent->preferred_installment_count ? ' ('.__(':count installments', ['count' => $selectedStudent->preferred_installment_count]).')' : '' }}.</p>
                        @endif
                        ...
                        <div x-show="paymentPlan === 'semester'" class="mt-4 space-y-3">
                            <div class="min-w-0">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Number of installments (full program length)') }}</label>
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __("Defaults to :count for this student's program. The total tuition is divided across this many semesters, even ones that don't exist yet.", ['count' => $expectedInstallmentCount]) }}</p>
                            </div>
                                ...
                                @forelse($semesterOptions as $semester)
                                    <label class="flex min-w-0 items-start gap-2 rounded-md border border-blue-100 bg-white p-3 text-sm dark:border-blue-900 dark:bg-gray-800 {{ $semester->end_date ? '' : 'opacity-60' }}">
                                        ...
                                        <span class="min-w-0">
                                            ...
                                            <span class="block text-xs text-gray-500 dark:text-gray-400">{{ $semester->end_date ? __('Due :date', ['date' => \Illuminate\Support\Carbon::parse($semester->end_date)->format('Y-m-d')]) : __('Missing semester end date') }}</span>
                                        </span>
                                    </label>
                                @empty
                                    <span class="text-sm text-blue-900 dark:text-blue-200">{{ __("No semesters are defined for this student's university yet.") }}</span>
                                @endforelse
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div class="min-w-0">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Transaction Date') }}</label>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div class="min-w-0">
                        ...
                        <input type="text" name="reference" value="{{ old('reference') }}" placeholder="{{ __('Receipt, invoice, voucher...') }}" class="mt-1 block w-full min-w-0 rounded-md border-gray-300 bg-white text-sm text-gray-900 focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                    </div>
                    <div class="min-w-0">
                ...
                <div x-show="recordType === 'invoice'" class="min-w-0">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Invoice Number') }}</label>
                    <input type="text" name="invoice_number" value="{{ old('invoice_number') }}" placeholder="{{ __('Auto for invoices') }}" class="mt-1 block w-full min-w-0 rounded-md border-gray-300 bg-white text-sm text-gray-900 focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                </div>

                <div x-show="['payment', 'discount'].includes(recordType)" class="min-w-0">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Apply Credit To Invoice') }}</label>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" x-show="!(recordType === 'discount' && discountMode === 'percentage')">{{ __('Used for payments and discounts. Currency and student must match the invoice.') }}</p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" x-show="recordType === 'discount' && discountMode === 'percentage'">{{ __("Required for percentage discounts — the percentage is applied to this invoice's full amount.") }}</p>
                </div>
