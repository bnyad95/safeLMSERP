                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($finalExamDrafts as $mark)
                                @php
                                    ...
                                    $secondTrialAllowed = ! is_null($firstTrialTotal) && $firstTrialTotal < 50;
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        @if(! is_null($firstTrialTotal))
                                            <p class="mt-1 text-xs {{ $firstTrialTotal >= 50 ? 'text-emerald-700' : 'text-red-700' }}">{{ $firstTrialTotal >= 50 ? 'Passed' : 'Failed' }} first trial</p>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        @if($activeTrial)
                                            <form method="POST" action="{{ route('marks.final-exam.store') }}" class="flex min-w-44 flex-col items-end gap-1" data-final-exam-autosave>
                                                ...
                                                <input type="number" name="score" min="0" max="100" step="0.01" required value="{{ is_null($activeScore) ? '' : number_format((float) $activeScore, 2, '.', '') }}" placeholder="{{ $activeTrial === 'second' ? 'Second score' : 'First score' }}" class="w-28 rounded-md border-gray-300 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500" data-final-score-input>
                                            </form>
                                        @else
