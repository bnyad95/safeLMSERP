        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $course->department?->college?->name ?? 'No college' }} / {{ $course->department?->name ?? 'No department' }}</p>
                        <h3 class="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $course->code }} - {{ $course->name }}</h3>
                    </div>
                    <a href="{{ route('marks.final-exam.index', request()->except(['final_exam_page'])) }}" class="inline-flex w-fit rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">Back to courses</a>
                </div>
            </section>
                    ...
                    <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
                        <thead class="bg-gray-50 dark:bg-gray-950">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Academic Scope</th>
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Class</th>
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Pre-final</th>
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">First Trial</th>
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Second Trial</th>
                                <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Final Mark</th>
                                <th class="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Entry</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @forelse($finalExamDrafts as $mark)
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                                    ...
                                    <td class="px-4 py-4">
                                        <p class="text-sm text-gray-700 dark:text-gray-300">{{ $section?->semester?->academic_year ?? 'No year' }}</p>
                                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $college?->name ?? 'No college' }}</p>
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        @if(! is_null($firstTrialTotal))
                                            <p class="mt-1 text-xs {{ $firstTrialTotal >= config('academics.pass_mark', 50) ? 'text-emerald-700 dark:text-emerald-300' : 'text-red-700 dark:text-red-300' }}">{{ $firstTrialTotal >= config('academics.pass_mark', 50) ? 'Passed' : 'Failed' }} first trial</p>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        @if($activeTrial)
                                            <form method="POST" action="{{ route('marks.final-exam.store') }}" class="flex min-w-44 flex-col items-end gap-1" data-final-exam-autosave>
                                                ...
                                                <label class="text-xs font-semibold {{ $activeTrial === 'second' ? 'text-red-700 dark:text-red-300' : 'text-blue-700 dark:text-blue-300' }}">{{ $activeTrial === 'second' ? 'Second trial active' : 'First trial' }}</label>
                                                <input type="number" name="score" min="0" max="{{ config('academics.final_exam_mark_max', 100) }}" step="0.01" required value="{{ is_null($activeScore) ? '' : number_format((float) $activeScore, 2, '.', '') }}" placeholder="{{ $activeTrial === 'second' ? 'Second score' : 'First score' }}" class="w-28 rounded-md border-gray-300 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100" data-final-score-input>
                                                @if($mark->submission_status === 'rejected' && ! is_null($activeScore))
                                                    <input type="text" name="change_reason" required maxlength="1000" placeholder="Correction reason" class="w-44 rounded-md border-gray-300 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                                                @endif
                                            </form>
                                        @else
                                            <span class="text-xs font-semibold text-emerald-700 dark:text-emerald-300">First trial passed</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No pre-final marks are waiting for final exam entry in this course.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
