<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">{{ __('Mark Submission Queue') }}</h2>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Review submitted and under-review marks, approve or reject them, then publish approved marks to students.') }}</p>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            @if($canManagePrefinalWindow)
                <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Pre-final Mark Entry') }}</h3>
                            @if($prefinalWindowUniversity)
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    {{ $prefinalWindowUniversity->name }} {{ __('is currently') }}
                                    <span class="font-semibold {{ $prefinalWindowUniversity->prefinal_marks_open ? 'text-emerald-700 dark:text-emerald-300' : 'text-red-700 dark:text-red-300' }}">{{ $prefinalWindowUniversity->prefinal_marks_open ? __('enabled') : __('disabled') }}</span>
                                    @if($prefinalWindowUniversity->prefinal_marks_open && $prefinalWindowUniversity->prefinalMarksOpener)
                                        ({{ __('enabled by :name', ['name' => $prefinalWindowUniversity->prefinalMarksOpener->name]) }}{{ $prefinalWindowUniversity->prefinal_marks_opened_at ? ' '.__('on :date', ['date' => $prefinalWindowUniversity->prefinal_marks_opened_at->format('Y-m-d')]) : '' }})
                                    @endif
                                </p>
                            @else
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('No university is associated with your account.') }}</p>
                            @endif
                        </div>
                        @if($prefinalWindowUniversity)
                            <form method="POST" action="{{ route('marks.prefinal-window.toggle') }}">
                                ...
                                <button type="submit" class="rounded-md px-4 py-2 text-sm font-semibold {{ $prefinalWindowUniversity->prefinal_marks_open ? 'border border-red-300 text-red-700 hover:bg-red-50 dark:border-red-800 dark:text-red-300 dark:hover:bg-red-900/20' : 'bg-emerald-600 text-white hover:bg-emerald-700' }}">
                                    {{ $prefinalWindowUniversity->prefinal_marks_open ? __('Disable pre-final entry') : __('Enable pre-final entry') }}
                                </button>
                            </form>
                        @endif
                    </div>
            ...
            <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <form method="GET" action="{{ route('marks.submission-queue') }}" class="grid gap-4 lg:grid-cols-6" x-data="{ showMore: {{ $hasAdvancedMarkFilters ? 'true' : 'false' }} }">
                    <div class="lg:col-span-3">
                        <label for="mark-search" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Search') }}</label>
                        <input id="mark-search" name="q" value="{{ $filters['q'] }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100" placeholder="{{ __('Student, ID, course, teacher') }}">
                    </div>

                    <div class="lg:col-span-2">
                        <label for="mark-status" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Queue status') }}</label>
                        <select id="mark-status" name="submission_status" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                            ...
                            @foreach(['submitted' => 'Submitted', 'under_review' => 'Under Review', 'approved' => 'Approved'] as $value => $label)
                                <option value="{{ $value }}" @selected($filters['submission_status'] === $value)>{{ __($label) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <a href="{{ route('marks.submission-queue') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Reset') }}</a>
                    </div>
                    ...
                    <div class="lg:col-span-6">
                        <button type="button" x-on:click="showMore = ! showMore" class="flex items-center gap-1 text-sm font-semibold text-indigo-700 hover:underline dark:text-indigo-400">
                            <span x-text="showMore ? @js(__('Fewer filters')) : @js(__('More filters'))"></span>
                    ...
                    <div x-show="showMore" x-cloak class="grid gap-4 border-t border-gray-100 pt-4 md:grid-cols-2 lg:col-span-6 lg:grid-cols-6 dark:border-gray-800">
                        <div>
                            <label for="mark-college" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('College') }}</label>
                        ...
                        <div>
                            <label for="mark-department" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Department') }}</label>
                        ...
                        <div>
                            <label for="mark-stage" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Stage') }}</label>
                        ...
                        <div>
                            <label for="mark-semester" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Semester') }}</label>
                        ...
                        <div>
                            <label for="mark-course" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Course') }}</label>
                        ...
                        <div>
                            <label for="mark-teacher" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Teacher') }}</label>
            ...
            <section class="grid gap-3 md:grid-cols-3">
                <div class="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
                    <p class="text-sm text-blue-800 dark:text-blue-200">{{ __('Submitted') }}</p>
                    <p class="mt-2 text-2xl font-semibold text-blue-950 dark:text-blue-100">{{ $queueStats['pending'] }}</p>
                    <p class="mt-1 text-xs text-blue-800 dark:text-blue-300">{{ __('Waiting for review') }}</p>
                </div>
                <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/20">
                    <p class="text-sm text-amber-800 dark:text-amber-200">{{ __('Under Review') }}</p>
                    <p class="mt-2 text-2xl font-semibold text-amber-950 dark:text-amber-100">{{ $queueStats['under_review'] }}</p>
                    <p class="mt-1 text-xs text-amber-800 dark:text-amber-300">{{ __('Can be approved or rejected') }}</p>
                </div>
                <div class="rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-800 dark:bg-emerald-900/20">
                    <p class="text-sm text-emerald-800 dark:text-emerald-200">{{ __('Ready to publish') }}</p>
                    <p class="mt-2 text-2xl font-semibold text-emerald-950 dark:text-emerald-100">{{ $queueStats['approved'] }}</p>
                    <p class="mt-1 text-xs text-emerald-800 dark:text-emerald-300">{{ __('Approved but not visible') }}</p>
                </div>
            </section>
            ...
            @if($canEnterFinalExam)
                <section class="rounded-lg border border-blue-200 bg-blue-50 p-5 shadow-sm dark:border-blue-800 dark:bg-blue-900/20">
                    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                            <h3 class="text-base font-semibold text-blue-950 dark:text-blue-100">{{ __('Final Exam Entry') }}</h3>
                            <p class="mt-1 text-sm text-blue-800 dark:text-blue-200">{{ __('Enter first-trial and eligible second-trial final exam marks from a scoped academic hierarchy.') }}</p>
                        </div>
                        <div class="flex flex-wrap items-center gap-3">
                            <span class="rounded-md bg-white px-3 py-2 text-sm font-semibold text-blue-900 shadow-sm dark:bg-gray-900 dark:text-blue-200">{{ $finalExamDraftCount }} {{ __('waiting') }}</span>
                            <a href="{{ route('marks.final-exam.index', request()->only(['q', 'academic_year', 'college_id', 'department_id', 'stage', 'semester_id', 'course_id'])) }}" class="rounded-md bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800 dark:bg-blue-600 dark:hover:bg-blue-500">{{ __('Open Final Exam Entry') }}</a>
                        </div>
                    </div>
                </section>
            @endif
                ...
                <div x-data="{ selected: [] }">
                        ...
                        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
                            <thead class="bg-gray-50 dark:bg-gray-950">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Course / Class') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Scope') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Mark') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Status') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Notes') }}</th>
                                    @if($canApprove || $canReject)<th class="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Action') }}</th>@endif
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                @forelse($pendingSubmissions as $mark)
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                                        ...
                                        <td class="px-4 py-4">
                                            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $course?->code ?? __('No code') }} - {{ $course?->name ?? '-' }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $section ? __('Group :code', ['code' => $section->section_code]).' / '.($section->semester?->name ?? __('Semester')) : __('No class linked') }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Teacher') }}: {{ $section?->teacher?->full_name ?? __('No teacher assigned') }}</p>
                                        </td>
                                        <td class="px-4 py-4">
                                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ $college?->name ?? __('No college') }}</p>
                                        ...
                                        <td class="px-4 py-4">
                                            ...
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Prefinal') }} {{ $mark->prefinal_mark ?? '-' }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('First trial') }} {{ is_null($mark->first_trial_final_exam) ? '-' : number_format((float) $mark->first_trial_final_exam, 1) }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Second trial') }} {{ is_null($mark->second_trial_final_exam) ? '-' : number_format((float) $mark->second_trial_final_exam, 1) }}</p>
                                        </td>
                                        <td class="px-4 py-4">
                                            @php
                                                ...
                                                $statusLabels = ['submitted' => 'Submitted', 'under_review' => 'Under Review', 'approved' => 'Approved', 'rejected' => 'Rejected', 'draft' => 'Draft'];
                                            @endphp
                                            <span class="rounded-md px-2 py-1 text-xs font-semibold {{ $colors[$mark->submission_status] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' }}">
                                                {{ __($statusLabels[$mark->submission_status] ?? ucfirst(str_replace('_', ' ', $mark->submission_status))) }}
                                            </span>
                                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ $mark->submitted_at?->diffForHumans() ?? __('No submit date') }}</p>
                                        </td>
                                                ...
                                                <div class="flex min-w-56 flex-col items-end gap-2">
                                                    @if($canApprove)
                                                        <form method="POST" action="{{ route('marks.approve') }}">
                                                            ...
                                                            <button type="submit" class="rounded-md bg-green-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-green-700">{{ __('Approve') }}</button>
                                                        </form>
                                                    @endif
                                                    @if($canReject)
                                                        <form method="POST" action="{{ route('marks.reject') }}" class="flex gap-2">
                                                            @csrf
                                                            <input type="hidden" name="mark_ids" value='@json([$mark->id])'>
                                                            <input type="text" name="notes" required placeholder="{{ __('Reason') }}" class="w-32 rounded-md border-gray-300 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                                                            <button type="submit" class="rounded-md bg-red-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-red-700">{{ __('Reject') }}</button>
                                                        </form>
                                                    @endif
                                                </div>
                                            </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ ($canApprove || $canReject) ? 8 : 7 }}" class="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('No pending marks match the current filters.') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    @if($canApprove || $canReject)
                        <div class="flex flex-wrap items-center gap-3 border-t border-gray-100 px-4 py-3 dark:border-gray-800" x-show="selected.length > 0">
                            <span class="text-sm text-gray-600 dark:text-gray-400" x-text="selected.length + ' ' + @js(__('selected'))"></span>
                            ...
                            @if($canApprove)
                                <form method="POST" action="{{ route('marks.approve') }}" @submit.prevent="submitWithIds($event, selected)" class="flex gap-2">
                                    ...
                                    <input type="text" name="notes" placeholder="{{ __('Reviewer notes (optional)') }}" class="rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                                    <button type="submit" class="rounded-md bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700">{{ __('Approve selected') }}</button>
                                </form>
                            @endif
                            ...
                            @if($canReject)
                                <form method="POST" action="{{ route('marks.reject') }}" @submit.prevent="submitWithIds($event, selected)" class="flex gap-2">
                                    @csrf
                                    <input type="hidden" name="mark_ids" :value="JSON.stringify(selected)">
                                    <input type="text" name="notes" placeholder="{{ __('Rejection reason') }}" required class="rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                                    <button type="submit" class="rounded-md bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">{{ __('Reject selected') }}</button>
                                </form>
                            @endif
                        </div>
                    @endif
            ...
            <section x-show="queueView === 'approved'" x-cloak class="mt-6 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-800">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">
                        {{ __('Approved') }} - {{ __('Ready to Publish') }}
                ...
                <div x-data="{ selectedApproved: [] }">
                        ...
                        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
                            <thead class="bg-gray-50 dark:bg-gray-950">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Approved By') }}</th>
                                    @if($canPublish)<th class="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ __('Action') }}</th>@endif
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                @forelse($approvedMarks as $mark)
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                                        ...
                                        <td class="px-4 py-4">
                                            ...
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $mark->reviewer_notes ?: __('No notes') }}</p>
                                        </td>
                                        @if($canPublish)
                                            <td class="px-4 py-4 text-right">
                                                <form method="POST" action="{{ route('marks.publish') }}" onsubmit="return confirm('{{ __('Publish this mark to the student?') }}')">
                                                    @csrf
                                                    <input type="hidden" name="mark_ids" value='@json([$mark->id])'>
                                                    <button type="submit" class="rounded-md bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700">{{ __('Publish') }}</button>
                                                </form>
                                            </td>
                                        @endif
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="{{ $canPublish ? 7 : 6 }}" class="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('No approved marks are ready to publish for the current filters.') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    @if($canPublish)
                        <div class="flex items-center gap-3 border-t border-gray-100 px-4 py-3 dark:border-gray-800" x-show="selectedApproved.length > 0">
                            <span class="text-sm text-gray-600 dark:text-gray-400" x-text="selectedApproved.length + ' ' + @js(__('selected'))"></span>
                            <form method="POST" action="{{ route('marks.publish') }}" @submit.prevent="if (confirm('{{ __('Publish selected marks to students?') }}')) submitWithIds($event, selectedApproved)">
                                @csrf
