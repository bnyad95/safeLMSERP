<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">{{ __('Edit Student') }}</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Update the student profile.') }}</p>
            </div>
            <a href="{{ route('students.show', $student) }}" class="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">{{ __('View Profile') }}</a>
        </div>
    </x-slot>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                    <div class="flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900">{{ __('Guardians') }}</h3>
                        <span class="text-sm text-gray-500">{{ __(':count saved', ['count' => $student->guardians->count()]) }}</span>
                    </div>
                    ...
                    <form action="{{ route('students.guardians.store', $student) }}" method="POST" class="mt-4 grid gap-4">
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('Relationship') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('Occupation') }}</label>
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">{{ __('Add Guardian') }}</button>
                        </div>
                    </form>
                            ...
                            <div class="rounded-md border border-gray-200 bg-gray-50 p-4">
                                <form action="{{ route('student-guardians.update', $guardian) }}" method="POST" class="grid gap-3">
                                    ...
                                    <div class="grid gap-3 md:grid-cols-2">
                                        ...
                                        <input type="text" name="phone" value="{{ $guardian->phone }}" placeholder="{{ __('Phone') }}" class="block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        <input type="email" name="email" value="{{ $guardian->email }}" placeholder="{{ __('Email') }}" class="block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        <input type="text" name="occupation" value="{{ $guardian->occupation }}" placeholder="{{ __('Occupation') }}" class="block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    ...
                                    <textarea name="address" rows="2" placeholder="{{ __('Address') }}" class="block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $guardian->address }}</textarea>
                                    <div class="flex justify-end">
                                        <button type="submit" class="text-sm font-medium text-blue-600 hover:underline">{{ __('Save') }}</button>
                                    </div>
                                </form>
                                <form action="{{ route('student-guardians.destroy', $guardian) }}" method="POST" class="mt-2 flex justify-end">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-sm font-medium text-red-600 hover:underline" onclick="return confirm('Remove this guardian?')">{{ __('Remove') }}</button>
                                </form>
                            </div>
                        @empty
                            <p class="rounded-md border border-dashed border-gray-300 p-4 text-sm text-gray-500">{{ __('No guardians have been added yet.') }}</p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                    <div class="flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900">{{ __('Documents') }}</h3>
                        <span class="text-sm text-gray-500">{{ __(':count uploaded', ['count' => $student->documents->count()]) }}</span>
                    </div>
                    ...
                    <form action="{{ route('students.documents.store', $student) }}" method="POST" enctype="multipart/form-data" class="mt-4 grid gap-4">
                            ...
                            <div>
                                ...
                                <select name="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                                    @foreach(['Admission form','ID document','Transcript','Medical record','Guardian consent','Other'] as $documentType)
                                        <option value="{{ $documentType }}">{{ __($documentType) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                                    @foreach(['Submitted','Verified','Rejected','Expired'] as $documentStatus)
                                        <option value="{{ $documentStatus }}">{{ __($documentStatus) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">{{ __('Upload Document') }}</button>
                        </div>
                    </form>
                        ...
                        @forelse($student->documents as $document)
                            <div class="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ __($document->type) }} / {{ __($document->status) }} / {{ $document->created_at->format('M d, Y') }}</p>
                                    @if($document->notes)
                                        <p class="mt-1 text-xs text-gray-500">{{ $document->notes }}</p>
                                    @endif
                                </div>
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('student-documents.download', $document) }}" class="text-sm font-medium text-blue-600 hover:underline">{{ __('Download') }}</a>
                                    <form action="{{ route('student-documents.destroy', $document) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-sm font-medium text-red-600 hover:underline" onclick="return confirm('Remove this document?')">{{ __('Remove') }}</button>
                                    </form>
                                </div>
                            </div>
                        @empty
                            <p class="p-4 text-sm text-gray-500">{{ __('No documents uploaded yet.') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
