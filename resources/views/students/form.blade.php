...
<div class="space-y-6 rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900 sm:p-6">
    <section>
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Student identity') }}</h3>
        <div class="mt-4 grid gap-5 md:grid-cols-2">
            <div>
                <label for="student-full-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Full name') }}</label>
            ...
            <div>
                <span class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Student ID') }}</span>
                <p class="mt-1 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200">{{ $isEdit ? $student->student_id : ($suggestedStudentId ?? __('Generated on save')) }}</p>
            </div>
            <div>
                <label for="student-email" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Email') }}</label>
            ...
            <div>
                <label for="student-phone" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Phone') }}</label>
            ...
            <div>
                <label for="student-national-id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('National ID / Passport number') }}</label>
                <input id="student-national-id" type="text" name="national_id" value="{{ old('national_id', $student->national_id ?? '') }}" placeholder="{{ __('Used to prevent duplicate student records') }}" class="mt-1 block w-full rounded-md border-gray-300 bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
    ...
    <section class="border-t border-gray-200 pt-6 dark:border-gray-700">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Academic organization') }}</h3>
        <div class="mt-4 grid gap-5 md:grid-cols-3">
            <div>
                <label for="student-university" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('University') }}</label>
            ...
            <div>
                <label for="student-college" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('College') }}</label>
            ...
            <div>
                <label for="student-department" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Department') }}</label>
            ...
            <div>
                <label for="student-stage" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Current stage') }}</label>
            ...
            <div>
                <label for="student-status" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Student status') }}</label>
                <select id="student-status" name="status" class="mt-1 block w-full rounded-md border-gray-300 bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" required>
                    @foreach(['Active', 'Inactive', 'Graduated'] as $statusOption)
                        <option value="{{ $statusOption }}" @selected(old('status', $student->status ?? 'Active') === $statusOption)>{{ __($statusOption) }}</option>
                    @endforeach
                </select>
    ...
    @unless($isEdit)
        <section class="border-t border-gray-200 pt-6 dark:border-gray-700">
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Tuition plan') }}</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __("Record how the student will pay tuition, billed automatically at the department's current rate — either one charge per semester as it happens, or the whole active year charged as a single invoice right away.") }}</p>
            <div class="mt-4 grid gap-5 md:grid-cols-2">
                <div>
                    <label for="student-payment-method" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Payment plan') }}</label>
                    <select id="student-payment-method" name="preferred_payment_method" class="mt-1 block w-full rounded-md border-gray-300 bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                        <option value="semester" @selected($selectedPaymentMethod === 'semester')>{{ __('Billed automatically each semester') }}</option>
                        <option value="full" @selected($selectedPaymentMethod === 'full')>{{ __('Full payment once') }}</option>
                    </select>
        ...
        <section class="border-t border-gray-200 pt-6 dark:border-gray-700">
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Scholarship') }}</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('If the organization is awarding this student a scholarship, set the percentage of tuition it covers. For example, 100% means the student owes nothing; 70% means the student still owes the remaining 30%. This is applied automatically to every tuition invoice generated for the student. Leave blank if there is no scholarship.') }}</p>
            <div class="mt-4 grid gap-5 md:grid-cols-2">
                <div>
                    <label for="student-scholarship-percentage" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Scholarship percentage') }}</label>
                    <div class="relative mt-1">
                        <input id="student-scholarship-percentage" type="number" min="0" max="100" step="0.01" name="scholarship_percentage" value="{{ old('scholarship_percentage') }}" placeholder="{{ __('e.g. 70') }}" class="block w-full rounded-md border-gray-300 bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
        ...
        <section class="border-t border-gray-200 pt-6 dark:border-gray-700">
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Student login') }}</h3>
            <div class="mt-4 grid gap-5 md:grid-cols-2">
                <div>
                    <label for="student-password" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Temporary password') }}</label>
                ...
                <div>
                    <label for="student-password-confirmation" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Confirm temporary password') }}</label>
    ...
    <section class="border-t border-gray-200 pt-6 dark:border-gray-700">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Admissions') }}</h3>
        <div class="mt-4 grid gap-5 md:grid-cols-2">
            <div>
                <label for="student-admission-status" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Admission status') }}</label>
                <select id="student-admission-status" name="admission_status" class="mt-1 block w-full rounded-md border-gray-300 bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                    @foreach(['Applicant', 'Admitted', 'Enrolled', 'Deferred', 'Withdrawn', 'Graduated'] as $admissionStatus)
                        <option value="{{ $admissionStatus }}" @selected(old('admission_status', $student->admission_status ?? 'Admitted') === $admissionStatus)>{{ __($admissionStatus) }}</option>
                    @endforeach
                </select>
                <x-input-error :messages="$errors->get('admission_status')" class="mt-2" />
            </div>
            <div>
                <label for="student-admission-date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Admission date') }}</label>
            ...
            <div>
                <label for="student-admission-type" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Admission type') }}</label>
                <input id="student-admission-type" type="text" name="admission_type" value="{{ old('admission_type', $student->admission_type ?? '') }}" placeholder="{{ __('Regular, transfer, scholarship') }}" class="mt-1 block w-full rounded-md border-gray-300 bg-white text-gray-900 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
            </div>
            <div>
                <label for="student-previous-school" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Previous school') }}</label>
            ...
            <div class="md:col-span-2">
                <label for="student-address" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Address') }}</label>
    ...
    <section class="border-t border-gray-200 pt-6 dark:border-gray-700">
        <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('Emergency contact') }}</h3>
        <div class="mt-4 grid gap-5 md:grid-cols-3">
            <div>
                <label for="student-emergency-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Name') }}</label>
            ...
            <div>
                <label for="student-emergency-relationship" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Relationship') }}</label>
            ...
            <div>
                <label for="student-emergency-phone" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Phone') }}</label>
    ...
    <div class="flex flex-col-reverse gap-3 border-t border-gray-200 pt-6 dark:border-gray-700 sm:flex-row sm:justify-end">
        <a href="{{ route('students.index') }}" class="inline-flex items-center justify-center rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">{{ __('Cancel') }}</a>
        <button type="submit" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">{{ $isEdit ? __('Update Student') : __('Create Student') }}</button>
    </div>
</div>
