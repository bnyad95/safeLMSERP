...
<div class="space-y-6 bg-white p-6 shadow-sm rounded-lg border border-gray-200">
    <div class="grid gap-6 md:grid-cols-2">
        ...
        <div>
            ...
            @if($isEdit)
                <p class="mt-1 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">{{ $student->student_id }}</p>
            @else
                <p class="mt-1 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">{{ $suggestedStudentId ?? 'Will be generated on save' }}</p>
            @endif
            <p class="mt-1 text-xs text-gray-500">This ID is generated by the system.</p>
        </div>
        <div>
