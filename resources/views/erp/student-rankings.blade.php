    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2 print:hidden">
                <a href="{{ route('bologna-definition.student-rankings.export', request()->query()) }}" class="inline-flex items-center justify-center rounded-md border border-emerald-300 bg-white px-4 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:bg-gray-900 dark:text-emerald-300">Export CSV</a>
                <button type="button" onclick="window.print()" class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">Print</button>
                <a href="{{ route('bologna-definition') }}" class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:hover:bg-gray-800">Back to Bologna Definition</a>
            </div>
        </div>
    </x-slot>
                                                        ...
                                                        @foreach($stage['years'] as $year)
                                                            <details class="rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                                                                <summary class="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800">
                                                                    ...
                                                                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ $year['total_students'] }} ranked</span>
                                                                </summary>
                                                                    ...
                                                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                                                        <thead class="bg-gray-50 dark:bg-gray-950">
                                                                            <tr>
                                                                                ...
                                                                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Credits</th>
                                                                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Progression</th>
                                                                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Graduation</th>
                                                                            </tr>
                                                                        </thead>
                                                                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                                                            @foreach($year['students'] as $student)
                                                                                <tr>
                                                                                    ...
                                                                                    <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $student['earned_credits'] }} this stage · {{ $student['lifetime_credits'] }} total</td>
                                                                                    <td class="px-4 py-3 text-sm">
                                                                                        @if(is_null($student['progression_eligible']))
                                                                                            <span class="text-amber-700 dark:text-amber-300">Policy missing</span>
                                                                                        @elseif($student['progression_eligible'])
                                                                                            <span class="text-emerald-700 dark:text-emerald-300">Eligible</span>
                                                                                        @else
                                                                                            <span class="text-red-700 dark:text-red-300">Needs {{ max(0, $student['required_progression_credits'] - $student['earned_credits']) }} credits</span>
                                                                                        @endif
                                                                                    </td>
                                                                                    <td class="px-4 py-3 text-sm">
                                                                                        ...
                                                                                        @elseif($student['graduation_eligible'])
                                                                                            <span class="text-emerald-700 dark:text-emerald-300">Eligible</span>
                                                                                        @else
                                                                                            <span class="text-gray-500 dark:text-gray-400">{{ max(0, $student['required_graduation_credits'] - $student['lifetime_credits']) }} credits remaining</span>
                                                                                        @endif
                                                                                    </td>
                                                                                </tr>
                                                                            @endforeach
