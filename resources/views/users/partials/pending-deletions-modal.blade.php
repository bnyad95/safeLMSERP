<x-modal name="pending-deletions" max-width="2xl">
    <div class="p-6">
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Pending Account Deletions</h2>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Self-requested account deletions waiting for review. Approving archives the account; rejecting keeps it active.</p>
        ...
        <div class="mt-4 max-h-[60vh] space-y-3 overflow-y-auto">
                ...
                <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="min-w-0">
                            ...
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Requested {{ $account->deletion_requested_at->format('Y-m-d H:i') }}</p>
                        </div>
                        @if($abilities['archive'])
                            <div class="flex shrink-0 gap-2">
                                <form action="{{ route('users.deletion.reject', $account) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">Reject</button>
                                </form>
                                <form action="{{ route('users.deletion.approve', $account) }}" method="POST" onsubmit="return confirm('Approve this deletion request? The account will be archived.')">
                                    @csrf
                                    <button type="submit" class="rounded-md bg-red-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-red-700">Approve</button>
                                </form>
                            </div>
                        @endif
                    </div>
            ...
            @empty
                <p class="py-6 text-center text-sm text-gray-500 dark:text-gray-400">No pending deletion requests.</p>
            @endforelse
        </div>
