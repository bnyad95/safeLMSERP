    ...
    <x-slot name="header">
        <div>
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">Closed academic years and the archived modules kept for historical access.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            <section class="grid gap-4 md:grid-cols-4">
                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <p class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Closed Years</p>
                    <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $years->count() }}</p>
                </div>
                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <p class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Archived Modules</p>
                    <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ number_format($years->sum('archived_modules')) }}</p>
                </div>
                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <p class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Students In Snapshots</p>
            ...
            <section class="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div class="border-b border-gray-200 px-5 py-4 dark:border-gray-800">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Academic Year Archive</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Each year stays visible after closure so students, teachers, and administrators can review old class records.</p>
                </div>
                <div class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($years as $year)
                        <a href="{{ route('academic-year-closures.archive.show', ['academic_year' => $year['academic_year']]) }}" class="grid gap-4 px-5 py-5 hover:bg-blue-50 dark:hover:bg-gray-800 lg:grid-cols-[minmax(220px,1fr)_repeat(5,minmax(110px,auto))] lg:items-center">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $year['universities']->join(', ') ?: 'Institution scope' }}</p>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-500">
                                    Closed {{ $year['closed_at'] ? $year['closed_at']->format('Y-m-d H:i') : 'date not recorded' }}
                                    @if($year['closed_by']->isNotEmpty())
                                        by {{ $year['closed_by']->join(', ') }}
                                    @endif
                                </p>
                            </div>
                            <div>
                            ...
                            <div>
                                <p class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Students</p>
                            ...
                            <div>
                                <p class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Closed Not Archived</p>
                    ...
                    @empty
                        <div class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                            No closed academic years yet. Closed years will appear here after the academic year closing workflow is completed.
                        </div>
                    @endforelse
                </div>
            </section>
