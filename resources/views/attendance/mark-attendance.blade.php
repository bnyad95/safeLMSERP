<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800">{{ __('Mark Attendance') }}</h2>
                <p class="text-sm text-gray-600">{{ $course->name }} ({{ $course->code }}){{ $section ? ' - '.__('Group :code', ['code' => $section->section_code]) : '' }} - {{ \Illuminate\Support\Carbon::parse($selectedDate)->format('l, d F Y') }}</p>
            </div>
            <div class="flex flex-wrap items-end gap-3">
                <form method="GET" action="{{ route('attendance.index', ['course' => $course->id]) }}" class="flex items-end gap-2">
                    ...
                    <label class="block">
                        <span class="text-xs font-semibold uppercase text-gray-500">{{ __('Date') }}</span>
                    ...
                    <button type="submit" class="rounded-md bg-gray-900 px-3 py-2 text-sm font-semibold text-white hover:bg-gray-800">{{ __('Open') }}</button>
                </form>
    ...
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <form id="attendance-form" class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-3 bg-gray-50 border-b flex items-center gap-4 flex-wrap">
                <span class="text-sm font-medium text-gray-600">{{ __('Mark all as:') }}</span>
                <button type="button" onclick="markAll('present')"
                        class="text-xs bg-green-100 text-green-700 px-3 py-1 rounded hover:bg-green-200 dark:bg-green-900/30 dark:text-green-200 dark:hover:bg-green-900/50">{{ __('All Present') }}</button>
                <button type="button" onclick="markAll('absent')"
                        class="text-xs bg-red-100 text-red-700 px-3 py-1 rounded hover:bg-red-200 dark:bg-red-900/30 dark:text-red-200 dark:hover:bg-red-900/50">{{ __('All Absent') }}</button>
                <button type="button" onclick="markAll('late')"
                        class="text-xs bg-yellow-100 text-yellow-700 px-3 py-1 rounded hover:bg-yellow-200">{{ __('All Late') }}</button>
            </div>
            @endif
            ...
            <table class="min-w-full divide-y divide-gray-100">
                ...
                <tbody class="divide-y divide-gray-100" id="attendance-table">
                                ...
                                <div class="flex gap-2">
                                    @foreach(['present' => ['bg-green-600','Present'], 'absent' => ['bg-red-600','Absent'], 'late' => ['bg-yellow-500','Late'], 'excused' => ['bg-blue-600','Excused']] as $status => [$color, $label])
                                        <label class="flex items-center gap-1 cursor-pointer">
                                            ...
                                            <span class="text-xs px-2 py-0.5 rounded-full text-white {{ $color }}">{{ __($label) }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-sm text-gray-500">{{ __('No students are enrolled in this class.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
document.getElementById('save-attendance')?.addEventListener('click', function() {
    ...
    .catch((error) => {
        ...
        fb.textContent = error?.message || @js(__('Failed to save attendance. Please try again.'));
        fb.classList.remove('hidden');
    });
});
</script>
