<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800">
            {{ __('Attendance - :student in :course', ['student' => $student->full_name, 'course' => $course->name]) }}
        </h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                <div class="mb-2 flex items-center justify-between">
                    <span class="text-sm font-medium text-gray-700">{{ __('Attendance Rate') }}</span>
                ...
                @if($pct < 75)
                    <p class="mt-2 text-xs text-red-600">{{ __('Below 75% threshold - student may face restrictions.') }}</p>
                @endif
            </div>

            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                <div class="border-b border-gray-100 px-6 py-4">
                    <h3 class="text-sm font-semibold text-gray-800">{{ __('Attendance Records') }}</h3>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($report['attendances'] as $record)
                            <tr>
                                ...
                                <td class="px-6 py-3">
                                    @php $colors = ['present'=>'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-200','absent'=>'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-200','late'=>'bg-yellow-100 text-yellow-700','excused'=>'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-200']; @endphp
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-8 text-center text-sm text-gray-500">{{ __('No attendance records.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
