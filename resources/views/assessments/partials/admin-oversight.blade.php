@php
    ...
    $assessmentCrumbs = collect([
        ['label' => __('Assessment Analytics'), 'href' => route('assessments.index')],
        ...
        $selectedSection ? ['label' => $selectedSection->course->code.' '.__('Group :code', ['code' => $selectedSection->section_code]), 'href' => null] : null,
    ])->filter()->values();
...
<nav class="flex flex-wrap items-center gap-2 text-sm" aria-label="{{ __('Assessment hierarchy') }}">
...
<section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm" x-data="{ showMore: {{ $advancedAssessmentFilterActive ? 'true' : 'false' }} }">
    <form method="GET" action="{{ route('assessments.index') }}" class="grid gap-4 lg:grid-cols-6">
        ...
        <div class="lg:col-span-2">
            <label for="assessment-search" class="block text-sm font-medium text-gray-700">{{ __('Search') }}</label>
            <input id="assessment-search" name="q" value="{{ $adminFilters['q'] }}" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="{{ __('Title or type') }}">
        </div>

        <div>
            <label for="assessment-course" class="block text-sm font-medium text-gray-700">{{ __('Course') }}</label>
        ...
        <div>
            <label for="assessment-status" class="block text-sm font-medium text-gray-700">{{ __('Status') }}</label>
            <select id="assessment-status" name="status" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                @foreach(['draft' => 'Draft', 'published' => 'Published', 'closed' => 'Closed'] as $value => $label)
                    <option value="{{ $value }}" @selected($adminFilters['status'] === $value)>{{ __($label) }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="lg:col-span-6" x-show="showMore" x-transition x-cloak>
            <div class="grid gap-4 border-t border-gray-100 pt-4 lg:grid-cols-5">
                <div>
                    <label for="assessment-semester" class="block text-sm font-medium text-gray-700">{{ __('Semester') }}</label>
                ...
                <div>
                    <label for="assessment-teacher" class="block text-sm font-medium text-gray-700">{{ __('Teacher') }}</label>
                ...
                <div>
                    <label for="assessment-type" class="block text-sm font-medium text-gray-700">{{ __('Type') }}</label>
                    <select id="assessment-type" name="type" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($adminFilterOptions['types'] as $type)
                            <option value="{{ $type }}" @selected($adminFilters['type'] === $type)>{{ __(ucfirst($type)) }}</option>
                        @endforeach
                    </select>
                </div>

                <div>
                    <label for="assessment-due-from" class="block text-sm font-medium text-gray-700">{{ __('Due from') }}</label>
                ...
                <div>
                    <label for="assessment-due-until" class="block text-sm font-medium text-gray-700">{{ __('Due until') }}</label>
        ...
        <div class="flex flex-wrap items-end gap-3 lg:col-span-6">
            ...
            <a href="{{ $resetUrl }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">{{ __('Reset') }}</a>
            <a href="{{ $exportUrl }}" class="rounded-md border border-emerald-300 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-800 hover:bg-emerald-100 dark:border-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-200 dark:hover:bg-emerald-900/50">{{ __('Export CSV') }}</a>
            @if($hasAdminFilters)
                <span class="text-sm text-gray-500">{{ __('Filters applied') }}</span>
            @endif
        </div>
    </form>
</section>
...
<section aria-label="Assessment analytics summary" class="space-y-4">
    @if($oversightStats['total'] === 0 && $hasAdminFilters)
        <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            {{ __('No assessments match the current filters in this scope.') }}
        </div>
    @endif

    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <p class="text-sm text-gray-500">{{ __('Total assessments') }}</p>
            <p class="mt-2 text-2xl font-semibold text-gray-900">{{ $oversightStats['total'] }}</p>
            <p class="mt-1 text-xs text-gray-500">{{ __('Across the current academic scope') }}</p>
        </div>
        <div class="rounded-lg border border-cyan-200 bg-cyan-50 p-4 dark:border-cyan-800 dark:bg-cyan-950/30">
            <p class="text-sm text-cyan-800 dark:text-cyan-300">{{ __('Submission rate') }}</p>
            <p class="mt-2 text-2xl font-semibold text-cyan-950 dark:text-cyan-100">{{ is_null($submissionRate) ? __('N/A') : number_format($submissionRate, 1).'%' }}</p>
            <p class="mt-1 text-xs text-cyan-800 dark:text-cyan-300">{{ __(':count submissions received', ['count' => $oversightStats['submitted']]) }}</p>
        </div>
        <div class="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
            <p class="text-sm text-emerald-800">{{ __('Grading rate') }}</p>
            <p class="mt-2 text-2xl font-semibold text-emerald-950">{{ is_null($gradingRate) ? __('N/A') : number_format($gradingRate, 1).'%' }}</p>
            <p class="mt-1 text-xs text-emerald-800">{{ __(':count submissions graded', ['count' => $oversightStats['graded']]) }}</p>
        </div>
        <div class="rounded-lg border border-amber-200 bg-amber-50 p-4">
            <p class="text-sm text-amber-800">{{ __('Needs attention') }}</p>
            <p class="mt-2 text-2xl font-semibold text-amber-950">{{ $oversightStats['ungraded'] + $oversightStats['overdue'] }}</p>
            <p class="mt-1 text-xs text-amber-800">{{ __(':ungraded ungraded, :overdue past due', ['ungraded' => $oversightStats['ungraded'], 'overdue' => $oversightStats['overdue']]) }}</p>
        </div>
    </div>
    ...
    <div class="grid gap-4 xl:grid-cols-3">
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            <div class="mb-5">
                <h3 class="font-semibold text-gray-900">{{ __('Status distribution') }}</h3>
                <p class="mt-1 text-sm text-gray-500">{{ __('Publication state of assessments') }}</p>
            </div>
            <div class="space-y-4">
                @foreach([
                    ['label' => __('Published'), 'value' => $oversightStats['published'], 'bar' => 'bg-emerald-500'],
                    ['label' => __('Draft'), 'value' => $oversightStats['draft'], 'bar' => 'bg-gray-500'],
                    ['label' => __('Closed'), 'value' => $oversightStats['closed'], 'bar' => 'bg-blue-500'],
                ] as $status)
                    <div>
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            <div class="mb-5">
                <h3 class="font-semibold text-gray-900">{{ __('Submission pipeline') }}</h3>
                <p class="mt-1 text-sm text-gray-500">{{ __('Progress from submission to grading') }}</p>
            </div>
            <div class="grid grid-cols-3 divide-x divide-gray-100 text-center">
                <div><p class="text-xl font-semibold text-gray-900">{{ $oversightStats['submitted'] }}</p><p class="mt-1 text-xs text-gray-500">{{ __('Submitted') }}</p></div>
                <div><p class="text-xl font-semibold text-emerald-700">{{ $oversightStats['graded'] }}</p><p class="mt-1 text-xs text-gray-500">{{ __('Graded') }}</p></div>
                <div><p class="text-xl font-semibold text-amber-700">{{ $oversightStats['ungraded'] }}</p><p class="mt-1 text-xs text-gray-500">{{ __('Ungraded') }}</p></div>
            </div>
            <div class="mt-6 space-y-4">
                <div><div class="mb-1.5 flex justify-between text-xs text-gray-600"><span>{{ __('Submitted') }}</span><span>{{ is_null($submissionRate) ? __('N/A') : number_format($submissionRate, 1).'%' }}</span></div><div class="h-2 overflow-hidden rounded bg-gray-100"><div class="h-full bg-cyan-500" style="width: {{ $submissionRate ?? 0 }}%"></div></div></div>
                <div><div class="mb-1.5 flex justify-between text-xs text-gray-600"><span>{{ __('Graded') }}</span><span>{{ is_null($gradingRate) ? __('N/A') : number_format($gradingRate, 1).'%' }}</span></div><div class="h-2 overflow-hidden rounded bg-gray-100"><div class="h-full bg-emerald-500" style="width: {{ $gradingRate ?? 0 }}%"></div></div></div>
            </div>
        </div>

        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            <div class="mb-5">
                <h3 class="font-semibold text-gray-900">{{ __('Assessment mix') }}</h3>
                <p class="mt-1 text-sm text-gray-500">{{ __('Items by assessment type') }}</p>
            </div>
            <div class="space-y-4">
                @forelse($oversightAnalytics['types'] as $type)
                    <div>
                        <div class="mb-1.5 flex items-center justify-between text-sm"><span class="capitalize text-gray-600">{{ __(ucfirst($type->type)) }}</span><span class="font-semibold text-gray-900">{{ $type->total }}</span></div>
                ...
                @empty
                    <p class="py-6 text-center text-sm text-gray-500">{{ __('No assessment types to analyze.') }}</p>
                @endforelse
            </div>
        </div>
    </div>
    ...
    <div class="grid gap-4 xl:grid-cols-2">
        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-100 px-5 py-4"><h3 class="font-semibold text-gray-900">{{ __('Needs attention') }}</h3><p class="mt-1 text-sm text-gray-500">{{ __('Overdue assessments and pending grading') }}</p></div>
            <div class="divide-y divide-gray-100">
                @forelse($oversightAnalytics['risk_items'] as $item)
                    ...
                    <a href="{{ $assessmentDrillUrl($item) }}" class="flex flex-col gap-2 px-5 py-4 hover:bg-gray-50 sm:flex-row sm:items-center sm:justify-between">
                        <div class="min-w-0"><p class="truncate text-sm font-semibold text-gray-900">{{ $item->title }}</p><p class="mt-1 text-xs text-gray-500">{{ $item->courseSection->course->code ?? __('Course') }} - {{ __('Group :code', ['code' => $item->courseSection->section_code]) }}</p></div>
                        <div class="flex shrink-0 flex-wrap gap-2">
                            @if($isPastDue)<span class="rounded-md bg-red-50 px-2 py-1 text-xs font-semibold text-red-700">{{ __('Past due') }}</span>@endif
                            @if($item->ungraded_submissions_count)<span class="rounded-md bg-amber-50 px-2 py-1 text-xs font-semibold text-amber-800">{{ __(':count ungraded', ['count' => $item->ungraded_submissions_count]) }}</span>@endif
                        </div>
                    </a>
                @empty
                    <p class="px-5 py-10 text-center text-sm text-gray-500">{{ __('Nothing needs attention in this scope.') }}</p>
                @endforelse
            </div>
        </div>

        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-100 px-5 py-4"><h3 class="font-semibold text-gray-900">{{ __('Upcoming deadlines') }}</h3><p class="mt-1 text-sm text-gray-500">{{ __('Published assessments due in the next seven days') }}</p></div>
            <div class="divide-y divide-gray-100">
                @forelse($oversightAnalytics['upcoming'] as $item)
                    <a href="{{ $assessmentDrillUrl($item) }}" class="flex items-center justify-between gap-4 px-5 py-4 hover:bg-gray-50">
                        <div class="min-w-0"><p class="truncate text-sm font-semibold text-gray-900">{{ $item->title }}</p><p class="mt-1 text-xs text-gray-500">{{ $item->courseSection->course->code ?? __('Course') }} - {{ __('Group :code', ['code' => $item->courseSection->section_code]) }} - {{ __(':count submitted', ['count' => $item->submissions_count]) }}</p></div>
                ...
                @empty
                    <p class="px-5 py-10 text-center text-sm text-gray-500">{{ __('No deadlines in the next seven days.') }}</p>
                @endforelse
            </div>
        </div>
    </div>
...
<div class="border-t border-gray-200 pt-6">
    <h3 class="text-lg font-semibold text-gray-900">{{ __('Browse assessments') }}</h3>
    <p class="mt-1 text-sm text-gray-500">{{ __('Narrow the analytics by college, department, stage, and class.') }}</p>
</div>

@if(! $selectedCollege)
    <section>
        <div class="mb-4"><h3 class="text-lg font-semibold text-gray-900">{{ __('Colleges') }}</h3><p class="mt-1 text-sm text-gray-500">{{ __('Select a college to review its assessments.') }}</p></div>
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @forelse($adminHierarchy['colleges'] as $card)
                <a href="{{ $collegeUrl($card['college']) }}" class="group rounded-lg border border-gray-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md">
                    <p class="text-xs font-semibold uppercase text-emerald-700">{{ __('College') }}</p><h4 class="mt-2 text-lg font-semibold text-gray-900 group-hover:text-emerald-800">{{ $card['college']->name }}</h4><p class="mt-1 text-sm text-gray-500">{{ $card['college']->code }}</p>
                    <p class="mt-5 border-t border-gray-100 pt-4 text-sm text-gray-600">{{ __(':count departments', ['count' => $card['department_count']]) }} - {{ __(':count assessments', ['count' => $card['assessment_count']]) }}</p>
                </a>
            @empty
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @forelse($adminHierarchy['departments'] as $card)
                <a href="{{ $departmentUrl($card['department']) }}" class="group rounded-lg border border-gray-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md">
                    ...
                    <p class="mt-5 border-t border-gray-100 pt-4 text-sm text-gray-600">{{ __(':count stages', ['count' => $card['grade_count']]) }} - {{ __(':count assessments', ['count' => $card['assessment_count']]) }}</p>
                </a>
            @empty
                <div class="rounded-lg border border-dashed border-gray-300 bg-white p-6 text-sm text-gray-500">{{ __('No departments match the selected scope and filters.') }}</div>
            @endforelse
        </div>
    </section>
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            @forelse($adminHierarchy['grades'] as $card)
                <a href="{{ $gradeUrl($card) }}" class="group rounded-lg border border-gray-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-md">
                    ...
                    <div class="mt-5 grid grid-cols-2 gap-4 border-t border-gray-100 pt-4 text-sm"><div><p class="text-xs text-gray-500">{{ __('Classes') }}</p><p class="mt-1 font-semibold text-gray-900">{{ $card['class_count'] }}</p></div><div><p class="text-xs text-gray-500">{{ __('Assessments') }}</p><p class="mt-1 font-semibold text-gray-900">{{ $card['assessment_count'] }}</p></div></div>
                </a>
            @empty
                <div class="rounded-lg border border-dashed border-gray-300 bg-white p-6 text-sm text-gray-500">{{ __('No stages match the selected scope and filters.') }}</div>
            @endforelse
        </div>
    </section>
@elseif(! $selectedSection)
    <section>
        <div class="mb-4"><h3 class="text-lg font-semibold text-gray-900">{{ __('Classes') }}</h3><p class="mt-1 text-sm text-gray-500">{{ __('Select a class to inspect its assessments.') }}</p></div>
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @forelse($adminHierarchy['classes'] as $section)
                <a href="{{ $classUrl($section) }}" class="group overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md">
                    <div class="min-h-28 bg-blue-700 p-5 text-white transition group-hover:bg-blue-800"><p class="text-lg font-semibold">{{ $section->course->name }}</p><p class="mt-1 text-sm text-blue-100">{{ $section->course->code }} - {{ __('Group :code', ['code' => $section->section_code]) }}</p></div>
                    <div class="grid grid-cols-2 divide-x divide-gray-100 px-4 py-4 text-sm"><div><p class="text-xs text-gray-500">{{ __('Students') }}</p><p class="mt-1 font-semibold text-gray-900">{{ $section->active_enrollments_count }}</p></div><div class="pl-4"><p class="text-xs text-gray-500">{{ __('Assessments') }}</p><p class="mt-1 font-semibold text-gray-900">{{ $section->filtered_assessment_items_count ?? $section->assessment_items_count }}</p></div></div>
                    <p class="border-t border-gray-100 px-4 py-3 text-xs text-gray-500">{{ __('Teacher') }}: {{ $section->teacher->full_name ?? __('Not assigned') }}</p>
                </a>
            @empty
                <div class="rounded-lg border border-dashed border-gray-300 bg-white p-6 text-sm text-gray-500">{{ __('No classes match this stage and filter set.') }}</div>
            @endforelse
        </div>
    </section>
@else
    <section>
        <div class="flex flex-col gap-4 border-b border-gray-200 pb-5 md:flex-row md:items-end md:justify-between">
            <div><p class="text-sm font-semibold text-blue-700">{{ $selectedGrade['label'] }}</p><h3 class="mt-1 text-lg font-semibold text-gray-900">{{ $selectedSection->course->name }} - {{ __('Group :code', ['code' => $selectedSection->section_code]) }}</h3><p class="mt-1 text-sm text-gray-500">{{ __('Class assessment analytics and read-only assessment details.') }}</p></div>
            <a href="{{ $gradeUrl($selectedGrade) }}" class="text-sm font-semibold text-blue-700 hover:underline">{{ __('Back to classes') }}</a>
        </div>
        ...
        <div class="mt-5 space-y-4">
            @forelse($assessmentItems as $item)
                <details id="assessment-{{ $item->id }}" class="group rounded-lg border border-gray-200 bg-white shadow-sm" @if($selectedAssessmentId === $item->id) open @endif>
                    <summary class="flex cursor-pointer list-none flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between">
                        <div><div class="flex flex-wrap items-center gap-2"><h4 class="text-base font-semibold text-gray-900">{{ $item->title }}</h4><span class="rounded-md bg-gray-100 px-2 py-1 text-xs font-semibold uppercase text-gray-700">{{ __(ucfirst($item->type)) }}</span><span class="rounded-md bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{{ $item->weight_percent }}%</span></div><p class="mt-1 text-sm text-gray-500">{{ __(ucfirst($item->status)) }} - {{ __(':count submitted', ['count' => $item->submissions_count]) }} - {{ __(':count graded', ['count' => $item->graded_submissions_count]) }}</p></div>
                        <span class="text-sm font-semibold text-blue-700"><span class="group-open:hidden">{{ __('Open details') }}</span><span class="hidden group-open:inline">{{ __('Close details') }}</span></span>
                    </summary>
            ...
            @empty
                <div class="rounded-lg border border-dashed border-gray-300 bg-white px-5 py-12 text-center text-sm text-gray-500">{{ __('No assessments match this class and filter.') }}</div>
            @endforelse
        </div>
