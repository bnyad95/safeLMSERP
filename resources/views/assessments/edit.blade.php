<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800">Edit Assessment</h2>
                <p class="text-sm text-gray-600">{{ $assessmentItem->title }}</p>
            </div>
            <a href="{{ route('assessments.index') }}" class="inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Back</a>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('assessment-items.update', $assessmentItem) }}" enctype="multipart/form-data" class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div>
                        ...
                        <select name="type" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            @foreach(['assignment' => 'Assignment', 'exam' => 'Exam', 'quiz' => 'Quiz', 'project' => 'Project', 'lab' => 'Lab'] as $value => $label)
                                <option value="{{ $value }}" @selected(old('type', $assessmentItem->type) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            @foreach(['draft' => 'Draft', 'published' => 'Published', 'closed' => 'Closed'] as $value => $label)
                                <option value="{{ $value }}" @selected(old('status', $assessmentItem->status) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="mt-4">
                    <label class="block text-sm font-medium text-gray-700">Replace instruction file</label>
                    ...
                    @if($assessmentItem->instruction_file_path)
                        <a href="{{ route('assessment-items.file', $assessmentItem) }}" class="mt-2 inline-flex text-sm font-medium text-blue-600 hover:underline">Download current file</a>
                    @endif
                </div>
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    <a href="{{ route('assessments.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</a>
