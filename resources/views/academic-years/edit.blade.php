<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Edit Academic Year</h2>
        ...
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('academic-years.update', $academicYear) }}" class="space-y-5 rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="status" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100">
                            <option value="upcoming" @selected(old('status', $academicYear->status) === 'upcoming')>Upcoming</option>
                            <option value="active" @selected(old('status', $academicYear->status) === 'active')>Active</option>
                        </select>
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Active years can only be completed through Academic Year Closing.</p>
                    </div>
                    <div>
                ...
                <div class="flex justify-end gap-3 border-t border-gray-100 pt-5 dark:border-gray-800">
                    <a href="{{ route('academic-years.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 dark:border-gray-700 dark:text-gray-200">Cancel</a>
                    <button class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">Update Academic Year</button>
                </div>
            </form>
        </div>
    </div>
