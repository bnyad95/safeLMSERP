    ...
    @if($canCreatePost)
        <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            <form method="POST" action="{{ route('class-stream.posts.store', $section) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
                    <label for="stream-body" class="text-sm font-semibold text-gray-900">{{ __('Share with your class') }}</label>
                    <textarea id="stream-body" name="body" rows="3" class="mt-2 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="{{ __('Post an announcement, reminder, or class update...') }}">{{ old('body') }}</textarea>
                    @error('body')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <label for="stream-attachment" class="block text-xs font-semibold text-gray-600">{{ __('Attach file') }}</label>
                    ...
                    <button type="submit" class="shrink-0 rounded-md bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800">{{ __('Post') }}</button>
                </div>
            </form>
        </section>
    @elseif(auth()->user()->hasRole('student'))
        <div class="rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm text-gray-600 shadow-sm">
            {{ __('Your teacher has limited new stream posts to teachers. You can still comment and react below.') }}
        </div>
    @endif
    ...
    <section class="space-y-4" aria-label="Class feed">
                ...
                <div class="p-5">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-gray-900">{{ $post->user->name ?? __('User') }}</p>
                                <p class="mt-0.5 text-xs text-gray-500">{{ $authorIsTeacher ? __('Teacher') : __('Student') }} - {{ $post->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    ...
                    @if($post->attachment_path)
                        <a href="{{ route('class-stream.posts.attachment', [$section, $post]) }}" class="mt-4 flex items-center justify-between gap-4 rounded-md border border-gray-200 bg-gray-50 px-4 py-3 hover:border-blue-300 hover:bg-blue-50">
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-gray-900">{{ $post->attachment_name ?? __('Attachment') }}</p>
                                <p class="mt-1 text-xs text-gray-500">{{ $post->attachment_mime ?? __('File attachment') }}</p>
                            </div>
                            <span class="shrink-0 text-sm font-semibold text-blue-700">{{ __('Download') }}</span>
                        </a>
                    @endif
                </div>
                    ...
                    @if($canInteract)
                        <form method="POST" action="{{ route('class-stream.reactions.toggle', [$section, $post]) }}">
                            ...
                            <button type="submit" class="text-sm font-semibold {{ $post->reacted_by_current_user ? 'text-blue-700' : 'text-gray-600 hover:text-blue-700' }}">
                                {{ $post->reacted_by_current_user ? __('Liked') : __('Like') }}{{ $post->reactions_count ? ' ('.$post->reactions_count.')' : '' }}
                            </button>
                        </form>
                    @else
                        <span class="text-sm text-gray-500">{{ $post->reactions_count }} {{ $post->reactions_count === 1 ? __('reaction') : __('reactions') }}</span>
                    @endif
                    <span class="text-sm text-gray-500">{{ $post->comments_count }} {{ $post->comments_count === 1 ? __('comment') : __('comments') }}</span>
                </div>
                ...
                <div class="border-t border-gray-100 bg-gray-50 px-5 py-4">
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <div class="min-w-0 rounded-lg bg-white px-3 py-2">
                                        <div class="flex flex-wrap items-baseline gap-x-2">
                                            <p class="text-xs font-semibold text-gray-900">{{ $comment->user->name ?? __('User') }}</p>
                    ...
                    @if($canInteract)
                        <form method="POST" action="{{ route('class-stream.comments.store', [$section, $post]) }}" class="flex gap-2">
                            @csrf
                            <label for="comment-{{ $post->id }}" class="sr-only">{{ __('Add class comment') }}</label>
                            <input id="comment-{{ $post->id }}" type="text" name="body" maxlength="2000" required class="min-w-0 flex-1 rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="{{ __('Add class comment...') }}">
                            <button type="submit" class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-100">{{ __('Comment') }}</button>
                        </form>
                    @endif
                </div>
            </article>
        @empty
            <div class="rounded-lg border border-dashed border-gray-300 bg-white px-5 py-12 text-center">
                <p class="text-sm font-semibold text-gray-700">{{ __('Nothing has been posted yet.') }}</p>
                <p class="mt-1 text-sm text-gray-500">{{ __('Class announcements and shared files will appear here.') }}</p>
            </div>
        @endforelse
    </section>
</div>
